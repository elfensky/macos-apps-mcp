...
def test_subject_search_under_1s():
    ...
    assert all(
        p["id"] and p["deeplink"].startswith("message://") for p in out["results"]
    )
...
def test_a_filed_message_search_cites_can_be_opened_and_its_attachments_listed():
    ...
    filed = [
        p
        for p in adapter.search(has_attachments=True, limit=50)["results"]
        if p.get("folder") and not p["folder"].rstrip("/").upper().endswith("INBOX")
    ]
    if not filed:
    ...
    assert isinstance(adapter.list_attachments(hit["folder"])["results"], list)
    # #155: and the id alone reaches the same message's attachments, no folder kept
    by_id = adapter.list_attachments(message_id=hit["id"])["results"]
    assert by_id and by_id[0]["id"].strip("<>") == hit["id"].strip("<>")
