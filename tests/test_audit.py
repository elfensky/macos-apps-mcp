...
def test_audit_write_appends(tmp_path, monkeypatch):
    ...
    au.audit_write({"tool": "create_event", "op": "create"})
    au.audit_write({"tool": "delete_note", "op": "delete"})
...
def test_audit_write_swallows_errors(tmp_path, monkeypatch):
    ...
    au.audit_write({"tool": "x"})  # must not raise
...
def test_audit_read_newest_first_and_since(tmp_path, monkeypatch):
    ...
    for ts, tool in [
        ...
        au.audit_write({"ts": ts, "tool": tool})
    assert [r["tool"] for r in au.audit_read()] == ["c", "b", "a"]
...
def test_audit_read_skips_malformed(tmp_path, monkeypatch):
    ...
    assert {r["tool"] for r in au.audit_read()} == {"ok", "ok2"}
