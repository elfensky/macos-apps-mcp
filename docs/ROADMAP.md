...
| **0.9.3 — Mail cleanup** | [#80](https://github.com/elfensky/macos-apps-mcp/issues/80) trash: soft + targeted permanent · [#140](https://github.com/elfensky/macos-apps-mcp/issues/140) same-mailbox dedupe + `mail_duplicates()` · [#153](https://github.com/elfensky/macos-apps-mcp/issues/153) cross-account dedupe · [#163](https://github.com/elfensky/macos-apps-mcp/issues/163) backup visibility | **Decisions settled with the operator 2026-08-04** (recorded on the issues): the gate is the **`allow-destructive` CLI switch** (allow-send pattern — a destructive tier whose tools are *absent* until a human flips it). **`empty_trash` is cut entirely** — it was the one verb that broke the plane's cap-25 backup math and threatened the dedupes' recovery story; emptying Trash stays a human job in Mail.app. So #80 is `trash_mail` (soft, write tier, on the #159 plane) + targeted permanent delete (≤25, fully backed up, `allow_lossy` for partial-bodied targets — the enforcement point already exists in `mail_recover.py`). The dedupes ship as the **`dedupe-mail` CLI** (thousands of ~0.1s deletes vs a 30s-capped serialized worker is the #119 shape, human-started), with a read-only **`mail_duplicates()`** MCP tool so a session can diagnose and point at it. Backup retention is **keep-forever, no pruning code** — #163 ships the visibility instead (`doctor()` line + size-threshold advisory); cleanup is a human act (shell today, the #126 dashboard's button later). Dedupe measurements re-verified 2026-08-03: 9,879 redundant same-mailbox rows (#140), ~3.9k cross-account (#153); winners differ (#140 lowest ROWID, #153 caller-named `keep_account`). Both act through **AppleScript, never sqlite**. |
