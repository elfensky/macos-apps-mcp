...
| Cut | Contents | Note |
|---|---|---|
| **0.9.1 — Mail organize** | [#78](https://github.com/elfensky/macos-apps-mcp/issues/78) mailbox hierarchy: list/create/move/archive · [#79](https://github.com/elfensky/macos-apps-mcp/issues/79) mark read/unread, flag (+colour) · [#154](https://github.com/elfensky/macos-apps-mcp/issues/154) `account` on `mail_body`/`mail_attachments` | #78's **read** half is nearly free — `build_overview_query` already reads the `mailboxes` table and the hierarchy is derivable from its URLs. Its **write** half must not repeat Mail.app's own bug: a cross-account move is copy → verify → delete-source, or it silently duplicates. #79 is small but quietly unrecoverable: nothing remembers which messages *were* unread. #154 is the same defect one layer down — a bare mailbox name matches by substring across every account, so `Archive` is ambiguous. |
| **0.9.2 — Mail cleanup** | [#80](https://github.com/elfensky/macos-apps-mcp/issues/80) trash: soft/permanent/empty · [#140](https://github.com/elfensky/macos-apps-mcp/issues/140) same-mailbox dedupe · [#153](https://github.com/elfensky/macos-apps-mcp/issues/153) cross-account dedupe | Same verb, same dry-run discipline — build together rather than doing trash three times. Both dedupes are measured on the dev machine: 9,881 redundant rows in one mailbox (#140), 3,945 cross-account copies from a migration (#153). They are separate tools because the winner differs — #140 keeps the lowest ROWID, #153 cannot infer which *account* should survive and makes the caller say. |
