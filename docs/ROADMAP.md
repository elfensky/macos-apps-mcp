...
| Cut | Contents | Note |
|---|---|---|
| **0.9.1 — Mail addressing** | [#155](https://github.com/elfensky/macos-apps-mcp/issues/155) the addressing triple + id-only resolution · [#156](https://github.com/elfensky/macos-apps-mcp/issues/156) stop under-answering silently | **Goes first because everything below consumes it.** `folder=` appears nowhere in `adapters/mail.py` — only the indexed path sets it — so `mail_needs_response()` hands back ids that `mail_body` cannot take, and `account` is on no read at all. #156 is the other half of the same honesty problem: `MAX_MAILS = 25` is a hard ceiling with no truncation signal, an unresolvable mailbox name returns `[]`, and the AppleScript fallback is inbox-only but shaped like a whole-store result. Both found by the [mail-vs-vault debate](#the-mail-vs-vault-debate); #155 closes #154. |
| **0.9.2 — Mail organize** | [#159](https://github.com/elfensky/macos-apps-mcp/issues/159) the recoverable destructive plane · [#78](https://github.com/elfensky/macos-apps-mcp/issues/78) mailbox hierarchy: list/create/move/archive · [#79](https://github.com/elfensky/macos-apps-mcp/issues/79) mark read/unread, flag (+colour) | **#159 goes first inside the cut** — #78's `move_mail` is the first destructive write, and every later one (#80, #140, #153) inherits dry-run/backup/log/undo from the plane instead of hand-rolling it: sqlite locates, *Python preserves*, AppleScript acts. #78's **read** half is nearly free — `build_overview_query` already reads the `mailboxes` table and the hierarchy is derivable from its URLs. Its **write** half must not repeat Mail.app's own bug: a cross-account move is copy → verify → delete-source, or it silently duplicates. It also *creates* citation rot by moving messages out from under stored folder tokens — which is why #155 lands first. #79 is small; its "nothing remembers which messages were unread" worry is answered by #155 carrying `unread` on the Pointer. |
...
| **0.9.4 — Mail extras** | [#81](https://github.com/elfensky/macos-apps-mcp/issues/81) save attachments to disk · [#85](https://github.com/elfensky/macos-apps-mcp/issues/85) statistics + export | #81 is blocked on #155: `mail_attachments` returns no message id, deeplink or folder today, so nothing can address the file it is asked to save. #85 rides the query plane `mail_overview` established, and depends on the dedup fix — stats over raw rows would be wrong by the same margin `mail_overview` was. |
