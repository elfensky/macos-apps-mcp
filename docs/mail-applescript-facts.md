...
| **`delete <mailbox>` is NOT scriptable — -10000 (`AppleEvent handler failed`) in every form**, by name, by `first mailbox whose name is …`, on a local or an account mailbox. Re-verified 2026-08-13, including with the mailbox's on-disk folder already deleted. | So `create_mailbox` has no scripted undo; removing a mailbox is a Mail.app UI action. Do not build a delete_mailbox tool against this. |
| **A LOCAL (On My Mac) mailbox CAN be removed outside the UI** — verified 2026-08-13 removing `171Probe`: quit Mail, `rm -rf` the `.mbox` under `~/Library/Mail/V10/<local-uuid>/`, AND remove its entry from `.mboxCache.plist` in the same directory (`plutil -remove mboxes.<name>`), then relaunch. Deleting only the `.mbox` is NOT enough: the mailbox keeps appearing in the sidebar and in `every mailbox` across restarts, fed by the cache plist — Mail does not reconcile against the missing folder and does not recreate it. | Empty-mailbox-only manoeuvre by policy (bytes in a removed `.mbox` are unrecoverably gone), and a stale `mailboxes` row lingers in the Envelope Index — inert, and Mail's own DB is not ours to write. This is a documented recovery path for probe litter, not a delete_mailbox tool: it requires quitting Mail, which no tool may do. |
